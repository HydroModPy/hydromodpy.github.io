...
.. list-table::
   ...
   * - Calibration depth
     ...
     - Done
   * - Storage evolution
     - First explicit DuckDB catalog schema migrations under
       ``hydromodpy/results/catalog/migrations/``, with round-trip tests and
       package-version pinning. The Zarr field store still has no in-place
       migration: a store at an older schema is regenerated by rerunning.
     - DuckDB done
   * - Application map
